...
#+begin_src elisp
  ...
  (defun generate-possible-editor-keybindings (editor-behavior-to-possible-keys)
    (let (res)
      (cond
       ((or (ht-empty? editor-behavior-to-possible-keys)
            (-some #'ht-empty? (ht-values editor-behavior-to-possible-keys)))
        nil)
       ((equal 1 (ht-size editor-behavior-to-possible-keys))
        (let ((action (ht-key editor-behavior-to-possible-keys)))
          (ht-map (lambda (possible-key _)
                    (ht (action possible-key)))
                  (ht-get editor-behavior-to-possible-keys action))))
       (t
        (let* ((action (ht-key editor-behavior-to-possible-keys))
              ...
              (ht ("move cursor up one line"
                  ...
                  ("lookup documentation"
                   (ht ("h" nil)
                       ...
                       ("d" nil)))))
             (list
              (ht ("move cursor up one line" "p")
                  ("move cursor down one line" "n")
                  ("move cursor left" "b")
                  ("move cursor right" "f")
